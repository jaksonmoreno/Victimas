<%@ Page Language="VB" AutoEventWireup="false" CodeFile="frmAdminComunitario.aspx.vb" Inherits="Victimas_frmAdminComunitario" %>
...
<body onkeydown="doSomething(event);">
    <script language="javascript" type="text/javascript">
        ...
        function ShowForm(Id_Comunitario, rowIndex) {
            var grid = window["<%= rgComunitario.ClientID%>"];
            var ventana1 = window.radopen("frmComunitario.aspx?opcion=view&Id_Comunitario=" + Id_Comunitario + "&n=" + (Math.random()), "UserListDialog");
            ventana1.Restore();
            ventana1.maximize();
            return false;
        }

        function ShowEditForm(Id_Comunitario, rowIndex) {
            var grid = window["<%= rgComunitario.ClientID%>"];
            var ventana4 = window.radopen("frmComunitario.aspx?opcion=edit&Id_Comunitario=" + Id_Comunitario + "&n=" + (Math.random()), "UserListDialog");
            ventana4.Restore();
            ventana4.maximize();
            return false;
        }
        ...
        function ShowInsertForm() {
            var ventana5 = window.radopen("frmComunitario.aspx?opcion=new" + "&n=" + (Math.random()), "UserListDialog");
            ventana5.Restore();
            ventana5.maximize();
            return false;
        }
        ...
        function refreshGrid(arg) {
            if (!arg) {
                var masterTable = $find("<%= rgComunitario.ClientID%>").get_masterTableView();
                masterTable.rebind();
            }
            else {
                var masterTable = $find("<%= rgComunitario.ClientID%>").get_masterTableView();
    ...
    <form id="form1" runat="server">
        ...
        <div>
            ...
            <telerik:RadGrid ID="rgComunitario" runat="server" AutoGenerateColumns="False" EnableAJAX="True"
                ...
                <MasterTableView AllowFilteringByColumn="True" CommandItemDisplay="Top" DataKeyNames="Id_Comunitario, Id_Comunitario" NoMasterRecordsText="No hay eventos para mostrar.">
                    <Columns>
                        ...
                        <telerik:GridBoundColumn DataField="Id_Comunitario" HeaderText="Ficha" UniqueName="Id_Comunitario"
                        ...
                        <telerik:GridTemplateColumn UniqueName="ver" AllowFiltering="False">
                            <ItemTemplate>
                                <asp:Image ID="VerComunitario" runat="server" ImageUrl="~/Imagenes/ver.gif" />
                            </ItemTemplate>
                            <ItemStyle Font-Bold="False" Font-Italic="False" Font-Overline="False" Font-Strikeout="False"
                                Font-Underline="False" HorizontalAlign="Center" Wrap="True" />
                        </telerik:GridTemplateColumn>
                    ...
                    <CommandItemSettings AddNewRecordText="Agregar Violencia Modalidad Comunitario" RefreshText="Actualizar" />
                    <CommandItemTemplate>
