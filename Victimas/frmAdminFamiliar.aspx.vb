...
Imports System.Data
Partial Class Victimas_frmAdminFamiliar
    ...
    Private oSituacion As New clsFamiliar
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            If clsSeguridad.ValidarPermisos(Session, "Familiar") = False Then
                Response.Redirect("../Administrador/Seguridad/Paginasinprivilegios.htm", True)
            End If

            clsSeguridad.FiltrarGrilla(Session, "Familiar", rgFamiliar)
            Dim Datos As New DataSet
            Dim objparametros As New Collection
            Dim ObjItems As New CmdParametro("@NombreFormulario", "Familiar", SqlDbType.Char, False, 100)
    ...
    Public ReadOnly Property ObtenerDatos() As DataSet
        ...
        Get
            ...
            Try
                ...
                objparametros.Add(prmt)
                Datos = clsBitacora.GetSelect("app.SpGetFamiliares", objparametros, Session, 126, False)   '<JPaez 09-12-2010>
    ...
    ''' <param name="e"></param>
    ''' <remarks>
    ''' Elimina un registro de la tabla tblViolenciaFamiliar al eliminarlo de la grilla.
    ''' </remarks>
    Protected Sub RadGrid1_DeleteCommand(ByVal source As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rgFamiliar.DeleteCommand
        Dim editedItem As GridEditableItem = CType(e.Item, GridEditableItem)

        Dim Indice As String = e.Item.OwnerTableView.DataKeyValues(e.Item.ItemIndex)("Id_Familiar")
        Dim Objparametros As New Collection
        Dim ObjItems As New CmdParametro("@Id_Familiar", Indice, SqlDbType.Int, True)
        Objparametros.Add(ObjItems)
        Dim update As String = clsBitacora.Update("spDeletetblViolenciaFamiliar", Objparametros, Session, 280, True)
        If update <> "0" Then
            rgFamiliar.Controls.Add(New LiteralControl("Hubo un Error en el Servidor de BD Id Error: " & update))
        End If
    End Sub
    ''' <summary>
    ...
    ''' </remarks>
    Protected Sub rgFamiliar_ItemCreated(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rgFamiliar.ItemCreated
        ...
        If TypeOf e.Item Is GridDataItem Then
            ...
            AbrirArchivo.Attributes("onClick") = String.Format("return ShowEditForm('{0}','{1}');", e.Item.OwnerTableView.DataKeyValues(e.Item.ItemIndex)("Id_Familiar"), e.Item.ItemIndex)
            AbrirArchivo.Style.Add("cursor", "hand")
            AbrirArchivo.ToolTip = "Haga click aqui para editar violencia familiar"


            Dim AbrirArchivoVer As Image = e.Item.FindControl("VerFamiliar")
            AbrirArchivoVer.Attributes("onClick") = String.Format("return ShowForm('{0}','{1}');", e.Item.OwnerTableView.DataKeyValues(e.Item.ItemIndex)("Id_Familiar"), e.Item.ItemIndex)
            AbrirArchivoVer.Style.Add("cursor", "hand")
            AbrirArchivoVer.ToolTip = "Haga click aqui para ver violencia familiar"
        ...
        Try
            ...
            If TypeOf sourceControl Is RadGrid Then
                Select Case eventArgument
                    Case "Rebind"
                        rgFamiliar.MasterTableView.SortExpressions.Clear()
                        rgFamiliar.MasterTableView.GroupByExpressions.Clear()
                        rgFamiliar.Rebind()
                    Case "RebindAndNavigate"
                        rgFamiliar.MasterTableView.SortExpressions.Clear()
                        rgFamiliar.MasterTableView.GroupByExpressions.Clear()
                        rgFamiliar.MasterTableView.CurrentPageIndex = rgFamiliar.MasterTableView.PageCount - 1
                        rgFamiliar.Rebind()
                End Select
            End If
        Catch ex As Exception
    ...
    ''' </remarks>
    Protected Sub rgFamiliar_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rgFamiliar.NeedDataSource

        'Me.RadGrid1.DataSource = ObtenerDatos(0, "", "", "").Tables("Datos")
        Me.rgFamiliar.DataSource = ObtenerDatos().Tables("Datos")
    ...
    Private Sub FilterEspanish()
        Dim Menu As GridFilterMenu = Me.rgFamiliar.FilterMenu
    ...
    Protected Sub btnFiltrar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnFiltrar.Click
        ...
        Me.rgFamiliar.DataSource = ObtenerDatos
        Me.rgFamiliar.DataBind()

    End Sub
    Protected Sub btnNoFiltrar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNoFiltrar.Click

        For Each column As GridColumn In rgFamiliar.MasterTableView.Columns
            column.CurrentFilterFunction = GridKnownFunction.NoFilter
            column.CurrentFilterValue = String.Empty
        Next

        rgFamiliar.MasterTableView.FilterExpression = String.Empty
        rgFamiliar.MasterTableView.Rebind()
