...
Imports System.Data
Partial Class Victimas_frmAdminIndividual
    ...
    Private oSituacion As New clsIndividual
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            If clsSeguridad.ValidarPermisos(Session, "Individual") = False Then
                Response.Redirect("../Administrador/Seguridad/Paginasinprivilegios.htm", True)
            End If

            clsSeguridad.FiltrarGrilla(Session, "Individual", rgIndividual)
            Dim Datos As New DataSet
            Dim objparametros As New Collection
            Dim ObjItems As New CmdParametro("@NombreFormulario", "Individual", SqlDbType.Char, False, 100)
    ...
    Public ReadOnly Property ObtenerDatos() As DataSet
        ...
        Get
            ...
            Try
                ...
                objparametros.Add(prmt)
                Datos = clsBitacora.GetSelect("app.SpGetIndividuales", objparametros, Session, 126, False)   '<JPaez 09-12-2010>
    ...
    End Property
    ' ''' <summary>
    ' ''' ChkActivo_CheckedChanged
    ' ''' </summary>
    ' ''' <param name="sender"></param>
    ' ''' <param name="e"></param>
    ' ''' <remarks>
    ' ''' Activa ó desactiva un registro de la tabla TblEventos
    ' ''' </remarks>
    'Protected Sub ChkActivo_CheckedChanged(ByVal sender As Object, ByVal e As System.EventArgs)
    '    If Session("Activar") Is Nothing Then
    '        Dim Registro As GridItem = CType(CType(sender, CheckBox).Parent.Parent, GridItem)
    '        Dim Indice As String = Registro.OwnerTableView.DataKeyValues(Registro.ItemIndex)("Id_Individual")
    '        Dim evento As Integer

    '        Dim ObjActivo As CheckBox = Registro.FindControl("chkActivo")
    '        Dim Objparametros As New Collection
    '        If ObjActivo.Checked = True Then
    '            evento = 282 'Activacion de informacion en tabla tblIndividual
    '        Else
    '            evento = 283 ' Incativacion de informacion en tabla tblIndividual
    '        End If
    '        Dim ObjItems As New CmdParametro("@Activo", ObjActivo.Checked, SqlDbType.Bit, False)
    '        Objparametros.Add(ObjItems)
    '        ObjItems = New CmdParametro("@Id_Individual", Indice, SqlDbType.Int, True)
    '        Objparametros.Add(ObjItems)
    '        Dim update As String = clsBitacora.Update("spUpdateActivaciontblViolenciaIndividual", Objparametros, Me.Session, evento, True)
    '    End If
    'End Sub
    ''' <summary>
    ...
    ''' <param name="e"></param>
    ''' <remarks>
    ''' Elimina un registro de la tabla tblIndividual al eliminarlo de la grilla.
    ''' </remarks>
    Protected Sub RadGrid1_DeleteCommand(ByVal source As Object, ByVal e As Telerik.Web.UI.GridCommandEventArgs) Handles rgIndividual.DeleteCommand
        Dim editedItem As GridEditableItem = CType(e.Item, GridEditableItem)

        Dim Indice As String = e.Item.OwnerTableView.DataKeyValues(e.Item.ItemIndex)("Id_Individual")
        Dim Objparametros As New Collection
        Dim ObjItems As New CmdParametro("@Id_Individual", Indice, SqlDbType.Int, True)
        Objparametros.Add(ObjItems)
        Dim update As String = clsBitacora.Update("spDeletetblViolenciaIndividual", Objparametros, Session, 280, True)
        If update <> "0" Then
            rgIndividual.Controls.Add(New LiteralControl("Hubo un Error en el Servidor de BD Id Error: " & update))
        End If
    End Sub
    ''' <summary>
    ...
    ''' </remarks>
    Protected Sub rgIndividual_ItemCreated(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridItemEventArgs) Handles rgIndividual.ItemCreated
        ...
        If TypeOf e.Item Is GridDataItem Then
            ...
            AbrirArchivo.Attributes("onClick") = String.Format("return ShowEditForm('{0}','{1}');", e.Item.OwnerTableView.DataKeyValues(e.Item.ItemIndex)("Id_Individual"), e.Item.ItemIndex)
            AbrirArchivo.Style.Add("cursor", "hand")
            AbrirArchivo.ToolTip = "Haga click aqui para editar violencia individual"


            Dim AbrirArchivoVer As Image = e.Item.FindControl("VerIndividual")
            AbrirArchivoVer.Attributes("onClick") = String.Format("return ShowForm('{0}','{1}');", e.Item.OwnerTableView.DataKeyValues(e.Item.ItemIndex)("Id_Individual"), e.Item.ItemIndex)
            AbrirArchivoVer.Style.Add("cursor", "hand")
            AbrirArchivoVer.ToolTip = "Haga click aqui para ver violencia individual"
        ...
        Try
            ...
            If TypeOf sourceControl Is RadGrid Then
                Select Case eventArgument
                    Case "Rebind"
                        rgIndividual.MasterTableView.SortExpressions.Clear()
                        rgIndividual.MasterTableView.GroupByExpressions.Clear()
                        rgIndividual.Rebind()
                    Case "RebindAndNavigate"
                        rgIndividual.MasterTableView.SortExpressions.Clear()
                        rgIndividual.MasterTableView.GroupByExpressions.Clear()
                        rgIndividual.MasterTableView.CurrentPageIndex = rgIndividual.MasterTableView.PageCount - 1
                        rgIndividual.Rebind()
                End Select
            End If
        Catch ex As Exception
    ...
    ''' </remarks>
    Protected Sub rgIndividual_NeedDataSource(ByVal source As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rgIndividual.NeedDataSource

        'Me.RadGrid1.DataSource = ObtenerDatos(0, "", "", "").Tables("Datos")
        Me.rgIndividual.DataSource = ObtenerDatos().Tables("Datos")
    ...
    Private Sub FilterEspanish()
        Dim Menu As GridFilterMenu = Me.rgIndividual.FilterMenu
    ...
    Protected Sub btnFiltrar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnFiltrar.Click
        ...
        Me.rgIndividual.DataSource = ObtenerDatos
        Me.rgIndividual.DataBind()

    End Sub
    Protected Sub btnNoFiltrar_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnNoFiltrar.Click

        For Each column As GridColumn In rgIndividual.MasterTableView.Columns
            column.CurrentFilterFunction = GridKnownFunction.NoFilter
            column.CurrentFilterValue = String.Empty
        Next

        rgIndividual.MasterTableView.FilterExpression = String.Empty
        rgIndividual.MasterTableView.Rebind()
